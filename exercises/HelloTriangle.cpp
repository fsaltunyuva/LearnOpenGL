...
int main()
{
    ...
    unsigned int fragmentShader2; // Shader object ID for fragment shader
    fragmentShader2 = glCreateShader(GL_FRAGMENT_SHADER); // Create a fragment shader
    glShaderSource(fragmentShader2, 1, &fragmentShaderSource2, NULL); // Attach source code
    glCompileShader(fragmentShader2); // Compile the fragment shader
    ...
    unsigned int shaderProgram2; // Shader program ID
    shaderProgram2 = glCreateProgram(); // Create a shader program object

    glAttachShader(shaderProgram2, vertexShader); // Attach vertex shader
    glAttachShader(shaderProgram2, fragmentShader2); // Attach fragment shader
    glLinkProgram(shaderProgram2); // Link shaders into a program
    ...
    */
    ...
    unsigned int VAO2; // Vertex Array Object, VAO
    glGenVertexArrays(1, &VAO2); // Generate a VAO ID
    glBindVertexArray(VAO2); // Bind the VAO to record vertex attribute configuration

    unsigned int VBO2; // Vertex Buffer Object, VBO (stores vertex data in GPU memory for efficiency)
    glGenBuffers(1, &VBO2); // Generate a buffer ID
    glBindBuffer(GL_ARRAY_BUFFER, VBO2); // Bind the buffer to the GL_ARRAY_BUFFER
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices2), vertices2, GL_STATIC_DRAW); // Copy vertex data to the buffer
