...
#pragma endregion
    ...
    Shader lightingShader("shaders/light_casters.vs", "shaders/light_casters.fs"); // objects that reflect light
...
#pragma endregion
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightingShader.use();
        // lightingShader.setVec3("light.position", -0.2f, -1.0f, -0.3f); // point light
        lightingShader.setVec3("light.position", camera.Position); // spot light (flashlight that follows camera)
        lightingShader.setVec3("light.direction", camera.Front); // spot light (flashlight that follows camera)
        lightingShader.setFloat("light.cutOff",   glm::cos(glm::radians(12.5f))); // spot light (flashlight that follows camera), will be used for comparison in fragment shader (cos value is enough for comparison)
        lightingShader.setFloat("light.outerCutoff", glm::cos(glm::radians(17.5f))); // spot light (flashlight that follows camera)
        ...
        glBindTexture(GL_TEXTURE_2D, diffuseMap); // bind that texture to the currently active texture unit

        lightingShader.setInt("material.specular", 1);
        glActiveTexture(GL_TEXTURE1); // activate the texture unit first before binding texture
        glBindTexture(GL_TEXTURE_2D, specularMap); // bind that texture to the currently active texture unit
        ...
        lightCubeShader.use();
        lightCubeShader.setVec3("lightSourceColor", glm::vec3(1.0f, 1.0f, 1.0f));
